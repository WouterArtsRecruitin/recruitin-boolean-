...
def show_search_for_functiegroep(processor: JobDiggerBooleanProcessor, fg_id: str) -> None:
...
Examples:
  ...
  %(prog)s -o /path/to/output              # Custom output directory
    ...
    parser.add_argument(
        "--input", "-i",
        type=Path,
        help="Input Excel bestand met vacatures"
    )
    parser.add_argument(
        "--output", "-o",
        type=Path,
    ...
    parser.add_argument(
        "--no-hf",
        action="store_true",
        help="Skip Hugging Face training data generatie"
    )
    ...
    parser.add_argument(
        "--show-taxonomy",
        action="store_true",
        help="Toon de functiegroep taxonomie"
    )
    parser.add_argument(
        "--search",
        type=str,
