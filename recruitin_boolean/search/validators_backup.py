...
class SearchConfig(BaseModel):
    ...
    search_types: List[str] = Field(
        default=[
            "breed", "specifiek", "lookalike", "competitor", 
            "skill_based", "open_to_work", "certification"
        ],
        description="Types of searches to generate"
    )
    max_results_estimate: int = Field(2000, ge=10, le=10000, description="Maximum expected results")
...
class SearchValidator:
    ...
    @staticmethod
    def validate_boolean_query(query: str, search_type: str, priority: str, expected_results: str) -> BooleanQuery:
        """
    ...
    def estimate_query_complexity(query: str) -> Dict[str, Any]:
        ...
        terms = len([t for t in re.split(r'\s+(?:AND|OR|NOT)\s+', query.upper()) if t.strip()])
