...
class SearchValidator:
    ...
    @staticmethod
    def validate_vacancy(title: str, company: str = None, location: str = None, fg_id: Optional[str] = None) -> VacancyInput:
    ...
    def estimate_complexity(query: str) -> float:
        ...
        factors = {
            ...
            'operators': min((query.upper().count('AND') + query.upper().count('OR') + query.upper().count('NOT')) / 20.0, 1.0),
    ...
    def check_functiegroep_coverage(query: str, functiegroep_terms: List[str]) -> float:
        ...
        matched_terms = sum(1 for term in functiegroep_terms if term.lower() in query_lower)
