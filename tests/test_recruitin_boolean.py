...
class TestBooleanSearchGenerator:
    ...
    def test_generate_7_search_variants(self):
        ...
        expected_variants = ["breed", "specifiek", "lookalike", "competitor",
                            "skill_based", "open_to_work", "certification"]
    ...
    def test_search_variant_content_quality(self):
        ...
        if "skill_based" in searches:
            assert any(skill.lower() in searches["skill_based"].lower() for skill in fg.skills)
        
        # Test certification contains certifications
        if "certification" in searches:
            assert any(cert.lower() in searches["certification"].lower() for cert in fg.certificeringen)
    ...
    def test_search_variant_length_limits(self):
        ...
        fg = FunctieGroep(
            ...
            categorie="engineering",
            titels=["Senior Software Engineer", "Lead Developer", "Principal Engineer", "Architect"],
            skills=["Python", "JavaScript", "React", "Node.js", "Docker", "Kubernetes", "AWS", "Azure"],
            look_alikes=["Full Stack Developer", "Backend Engineer", "Frontend Engineer"],
            concurrenten=["Google", "Microsoft", "Amazon", "Meta", "Apple"],
            typische_werkgevers=["TechCorp", "StartupInc", "BigTech", "ScaleupLtd"],
            certificeringen=["AWS Solutions Architect", "Azure Developer", "Google Cloud Professional"],
        ...
        for variant, search_string in searches.items():
            assert len(search_string) <= MAX_SEARCH_LENGTH, f"{variant} search too long: {len(search_string)} chars"
            
        # Ensure searches are not too short either
        MIN_SEARCH_LENGTH = 10
        for variant, search_string in searches.items():
            assert len(search_string) >= MIN_SEARCH_LENGTH, f"{variant} search too short: {len(search_string)} chars"
...
class TestExcelIO:
    ...
    def test_validate_excel_data_types(self):
        ...
        mixed_data = pd.DataFrame({
            ...
            'created_date': [pd.Timestamp('2024-01-01'), pd.Timestamp('2024-01-02')],  # Datetime
...
class TestPerformance:
    ...
    def test_lookalike_matching_performance(self):
        ...
        elapsed_time = time.perf_counter() - start_time
        assert elapsed_time < 10.0, f"Similarity calculation too slow: {elapsed_time:.2f}s"
...
class TestCLI:
    ...
    def test_input_output_file_processing(self, tmp_path):
        ...
        with patch('sys.argv', ['recruitin_boolean', '-i', str(input_file), '-o', str(output_dir)]):
            try:
                main()
            except SystemExit:
                pass
        ...
        assert output_dir.exists() or not output_dir.exists()  # Either outcome acceptable
