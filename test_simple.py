...
class TestBooleanSearchGenerator:
    def test_generate_7_search_variants(self):
        ...
        expected_variants = ["breed", "specifiek", "lookalike", "competitor", "skill_based", "open_to_work", "certification"]
    ...
    def test_search_variant_content_quality(self):
        ...
        for variant, search in searches.items():
            ...
            assert 10 <= len(search) <= 1000, f"{variant} search should be 10-1000 characters"
    ...
    def test_search_variant_length_limits(self):
        ...
        functiegroep_data = {
            ...
            "functietitels": ["Senior Software Engineer", "Lead Developer", "Technical Architect", "Full Stack Engineer"],
            "vaardigheden": ["Python", "JavaScript", "TypeScript", "React", "Node.js", "PostgreSQL", "MongoDB", "AWS"],
            "certificaten": ["AWS Solutions Architect", "Google Cloud Professional", "Kubernetes Administrator", "Scrum Master"],
        ...
        for variant, search in searches.items():
            ...
            assert len(search) <= 1000, f"{variant} search exceeds LinkedIn 1000 char limit: {len(search)}"
            
            # Meaningful minimum
            assert len(search) >= 10, f"{variant} search too short to be meaningful: {len(search)}"
